...
FOR e IN (
			SELECT CONTRACTID FROM CONTRACTS WHERE  ENDDATE>SYSDATE OR ENDDATE IS NULL
          ) LOOP 

		INSERT INTO taps_movies VALUES (e.contractId,(x),99,'Titanic');
		
		INSERT INTO taps_series VALUES (e.contractId,(x),99,'House',1,1);
		INSERT INTO taps_series VALUES (e.contractId,(x),99,'Sherlock',1,1);
		INSERT INTO taps_series VALUES (e.contractId,(x),99,'Columbo',1,1);
...
FOR g IN (
SELECT * FROM (SELECT B.ACTOR, COUNT('X') USA_MOVIES 
         FROM (SELECT MOVIE_TITLE FROM MOVIES WHERE COUNTRY='USA') A 
              JOIN CASTS B 
         ON (A.MOVIE_TITLE=B.TITLE) 
         GROUP BY B.ACTOR 
...
PROCEDURE PR_RESET(N NUMBER) IS
  BEGIN
	...
	DELETE FROM taps_movies WHERE (pct=99 AND title='Titanic');

	DELETE FROM taps_series WHERE (pct=99 AND title='House' AND season=1 AND episode=1);
	DELETE FROM taps_series WHERE (pct=99 AND title='Columbo' AND season=1 AND episode=1);
	DELETE FROM taps_series WHERE (pct=99 AND title='Sherlock' AND season=1 AND episode=1);  
